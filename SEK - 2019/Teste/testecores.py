...
sound.speak('ok')        
c = open('testecor.txt','w+')
c.close()
for i in range(7):
    sound.beep()
    sleep(5)
    sound.beep()
    rgb_cru_e = [cor_esq.red,cor_esq.green,cor_esq.blue] #colocar na lista as componentes
    rgb_e = escalarRGB(rgb_cru_e,rgbmax_e)               #escalo pra 255
    hsv_e = RGBtoHSV(rgb_e)
    rgb_cru_d = [cor_dir.red,cor_dir.green,cor_dir.blue] #colocar na lista as componentes
    rgb_d = escalarRGB(rgb_cru_d,rgbmax_d)               #escalo pra 255
    hsv_d = RGBtoHSV(rgb_d)
    c = open('testecor.txt','a')
    n = str(i)
    c.write(n)
    c.write(') RGB CRU E:')
    c.write(str(rgb_cru_e))
    c.write(' RGB CRU D:')
    c.write(str(rgb_cru_d))
    c.write('\n   RGB  E:')
    c.write(str(rgb_e))
    c.write('  RGB  D:')
    c.write(str(rgb_d))
