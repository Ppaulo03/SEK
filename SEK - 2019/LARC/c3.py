...
tamanhos=['azul','vermelho','azul','vermelho']
item_lista = 0
# Começo
...
while True:
    while meeting_area: #começa aleatório, termina virado pro preto
        ...
        if cor('esq')=='vazio' or cor('dir')=='vazio':
            ...
            a = testar_preto()
            log = open('log.txt','a')
            log.write('Testar se está na faixa preta\n')
            log.close()
            if a:
                log = open('log.txt','a')
                log.write('Faixa preta\n')
                log.close()
                while cor('esq')!='branco' and cor('dir')!='branco':
                    log = open('log.txt','a')
                    log.write('Esquerdo e direito dif de branco, indo pra trás\n')
                    log.close()
                    steering_pair.on(0,-15)
                else:
                    log = open('log.txt','a')
                    log.write('Parou, voltou um pouco e alinhou\n')
                    log.close()
                    steering_pair.off()
                ...
                aprender_cores = True
                log = open('log.txt','a')
                log.write('Acabou a loc inicial, ir aprender cores\n')
                log.close()
            else:
                log.write('Rampa, volta e meia volta\n')
                steering_pair.on_for_degrees(0,-10,350)
